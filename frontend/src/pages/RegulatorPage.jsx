    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
        {[
          ...
          { label: "High Risk",     value: data.summary.highRiskCount || 0, color: "text-red-400" },
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-gray-500 text-xs border-b border-gray-800">
                  {["Time","Wallet","Symbol","Qty","Amount","AML","Status","Action"].map(h => (
                ...
                {data.recentTrades.map(t => (
                  <tr key={t.id} className={`hover:bg-gray-800/50 transition-colors ${t.flagged ? "bg-red-500/5" : ""}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-white font-mono text-xs">{t.userAddress ? shortHash(t.userAddress) : "—"}</span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {t.aml && (
                        <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border ${AML_COLOR[t.aml.level] || AML_COLOR.LOW}`}>
                          {t.aml.level}
                          {t.aml.score > 0 && <span className="text-[10px]">({t.aml.score})</span>}
                        </span>
                      )}
                      {t.aml?.reasons?.length > 0 && (
                        <p className="text-[10px] text-gray-600 mt-0.5">{t.aml.reasons[0]}</p>
                      )}
                    </td>
                    <td className="px-4 py-3">
                        ...
                        <p className="text-[10px] text-gray-600 mt-0.5">
                          {EXPLORER_URL ? (
                            <a href={`${EXPLORER_URL}/tx/${t.txHash}`} target="_blank" rel="noopener noreferrer"
                               className="text-green-400 hover:text-green-300">{shortHash(t.txHash)} ↗</a>
                          ) : shortHash(t.txHash)}
                        </p>
                      )}
                    </td>
            ...
            <div key={t.id} className="card p-4 border-red-500/30 bg-red-500/5">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-1">Wallet: {t.userAddress ? shortHash(t.userAddress) : t.id}</p>
                </div>
